...
![Database - Instance](/medias/post/mysql/Database-Instance.jpg)

### MySQL的架构
MySQL 从第一个版本发布到现在已经有了 20 多年的历史，在这么多年的发展和演变中，整个应用的体系结构变得越来越复杂：
![Logical-View-of-MySQL-Architecture](/medias/post/mysql/Logical-View-of-MySQL-Architecture.jpg)
...
![Tablespace-segment-extent-page-row](/medias/post/mysql/Tablespace-segment-extent-page-row.jpg)

同一个数据库实例的所有表空间都有相同的页大小；默认情况下，表空间中的页大小都为 16KB，当然也可以通过改变 innodb_page_size 选项对默认大小进行修改，需要注意的是不同的页大小最终也会导致区大小的不同：

![Relation Between Page Size](/medias/post/mysql/Relation Between Page Size - Extent Size.png)
...
![FRM](/medias/post/mysql/frm-and-ibd-file.jpg)
...
```
当我们使用上面的代码创建表时，会在磁盘上的 `datadi`r 文件夹中生成一个 `test_frm.frm` 的文件，这个文件中就包含了表结构相关的信息：

![](/medias/post/mysql/frm-file-hex.png)
...
![Antelope-Barracuda-Row-Format](/medias/post/mysql/Antelope-Barracuda-Row-Format.jpg)

>Antelope 是 InnoDB 最开始支持的文件格式，它包含两种行格式 Compact 和 Redundant，它最开始并没有名字；Antelope 的名字是在新的文件格式 Barracuda 出现后才起的，Barracuda 的出现引入了两种新的行格式 Compressed 和 Dynamic；InnoDB 对于文件格式都会向前兼容，而官方文档中也对之后会出现的新文件格式预先定义好了名字：Cheetah、Dragon、Elk 等等。

两种行记录格式 Compact 和 Redundant 在磁盘上按照以下方式存储：
![COMPACT-And-REDUNDANT-Row-Format](/medias/post/mysql/COMPACT-And-REDUNDANT-Row-Format.jpg)
...
![](/medias/post/mysql/Row-Overflow.jpg)

但是当我们使用新的行记录格式 Compressed 或者 Dynamic 时都只会在行记录中保存 20 个字节的指针，实际的数据都会存放在溢出页面中。

![](/medias/post/mysql/Row-Overflow-in-Barracuda.jpg)
...
![](/medias/post/mysql/InnoDB-B-Tree-Node.jpg)
...
![](/medias/post/mysql/Infimum-Rows-Supremum.jpg)
...
![](/medias/post/mysql/Clustered-Index.jpg)
...
![](/medias/post/mysql/Optimistic-Pessimistic-Locks.jpg)
...
![](/medias/post/mysql/Shared-Exclusive-Lock.jpg)
...
![](/medias/post/mysql/Lock-Type-Compatibility-Matrix.jpg)
...
![](/medias/post/mysql/Deadlocks.jpg)
...
![](/medias/post/mysql/Transaction-Isolation-Matrix.jpg)

### 脏读
>在一个事务中，读取了其他事务未提交的数据。

当事务的隔离级别为 READ UNCOMMITED 时，我们在 SESSION 2 中插入的未提交数据在 SESSION 1 中是可以访问的。
![](/medias/post/mysql/Read-Uncommited-Dirty-Read.jpg)
...
![](/medias/post/mysql/2019-07-16-Read-Commited-Non-Repeatable-Read.jpg)
...
！[](/medias/post/mysql/Repeatable-Read-Phantom-Read.jpg)

这种现象在数据库中就被称作幻读，虽然我们使用查询语句得到了一个空的集合，但是插入数据时却得到了错误，好像之前的查询是幻觉一样。

在标准的事务隔离级别中，幻读是由更高的隔离级别 SERIALIZABLE 解决的，但是它也可以通过 MySQL 提供的 Next-Key 锁解决：
![](/medias/post/mysql/Repeatable-with-Next-Key-Lock.jpg)
